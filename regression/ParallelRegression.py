...
def readData(input_file, spark_context):
    """
    Read data from an input file and return rdd containing pairs of the form: (x,y)
...
def F(data, beta, lam=0):
    ...
    """
    n = data.count()
    MSE = 1. / n * data.map(lambda (x, y): f(x, y, beta)).reduce(add)
    Reg = lam * np.dot(beta, beta)
...
def gradient(data, beta, lam=0):
    ...
    """
    n = data.count()
    grad = 1. / n * data.map(lambda (x, y): localGradient(x, y, beta)).reduce(add)
...
def train(data, beta_0, lam, max_iter, eps):
    ...
    while k < max_iter and eps < gradNorm:
        grad = gradient(data, beta, lam)
        fun = lambda x: F(data, x, lam)
        gamma = lineSearch(fun, beta, grad)
        beta = beta - gamma * grad
        gradNorm = np.linalg.norm(grad)
        reg_mse = F(data, beta, lam)
        end = time() - start;
        k += 1
        print "k = {0}\t time = {1}\t F(βk) = {2}\t ||∇F(β_k)||2 = {3}".format(k, end, reg_mse, gradNorm)
