...
"""
USAGE EXAMPLE: python MFspark.py small_data 5 --N 40 --gain 0.0001 --pow 1.0 --maxiter 20 --d 4
"""
...
def SE(joinedRDD):
    ...
    """
    return joinedRDD.map(lambda (i, j, delta, u, v): delta ** 2).reduce(lambda x, y: x + y)
...
def normSqRDD(profileRDD, param):
    """ Receives as input an RDD of profiles (e.g., U) as well as a parameter (e.g., λ) and computes the square of norms:
        ...
        λ Σ_i ||ui||_2^2
    """
    return param * profileRDD.map(lambda (i, u): np.dot(u, u)).reduce(lambda x, y: x + y)
...
def adaptU(joinedRDD, gamma, lam, N):
    ...
    """
    resRDD = joinedRDD.map(lambda (i, j, delta, ui, vj): (i, (ui, 2 * delta * vj + 2 * lam * ui))) \
        .reduceByKey(lambda (ui1, grad1), (ui2, grad2): (ui1, grad1 + grad2), numPartitions=N) \
        .mapValues(lambda (ui, grad): ui - gamma * grad)
    return resRDD
...
def adaptV(joinedRDD, gamma, mu, N):
    ...
    """
    resRDD = joinedRDD.map(lambda (i, j, delta, ui, vj): (j, (vj, 2 * delta * ui + 2 * mu * vj))) \
        .reduceByKey(lambda (ui1, grad1), (ui2, grad2): (ui1, grad1 + grad2), numPartitions=N) \
        .mapValues(lambda (vj, grad): vj - gamma * grad)
    return resRDD
